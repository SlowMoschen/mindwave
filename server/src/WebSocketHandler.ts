...
export class WebSocketHandler {
  ...
  private Initialize() {
    this._SocketServer.use((socket, next) => {
      socket.onAny((event) => {
        Logger.logSocketEvent(`${socket.id} - ${socket.handshake.address} - ${new Date().toISOString()} - ${event}`);
      });
      next();
    });
  ...
  private HandleConnection(client: Socket) {
    ...
    client.on(SOCKET_EVENTS.ROOM_EVENTS.CREATE, (data: RoomConfigurationDTO) => this.HandleRoomCreation(client, data));

    client.on(SOCKET_EVENTS.ROOM_EVENTS.JOIN, (data: JoinRoomDTO) => this.HandleRoomJoin(client, data));

    client.on(SOCKET_EVENTS.ROOM_EVENTS.LEAVE, (roomName: string) => this.HandleRoomLeave(client, roomName));
  }
